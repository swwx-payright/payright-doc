...
如果您通过Payright平台代申请开通渠道或者自行填写了渠道参数之后，就可以开始集成开发了。
...
1. **生成和配置RSA签名密钥**
	...
	Payright的数据传输使用RSA签名来验证数据可靠性，因此需要设置RSA签名秘钥。

	Payright Server SDK中提供了生成RSA密钥对的工具RSAKeyGenerateUtil，您可以直接用它来生成一对公私钥，并将公钥填写到Payright网站“个人中心”-“基本信息”-“开发信息”中。
...
2. **设置SecretKey和签名秘钥**
	...
	在Payright网站配置了商户公钥后，还需要在程序中配置以下三要素：
	* 商户私钥：和商户公钥对应的RSA私钥，上一步已经生成
	* Payright公钥：向Payright传输数据时需要使用Payright公钥对数据签名，请到Payright网站“基本信息”-“开发信息”-“Payright公钥”查看
	* Secret Key：Payright提供给商户的唯一标识，请到Payright网站“基本信息”-“开发信息”-“Live Secret Key”查看
...
3. 构造请求参数并发起支付请求，并解析服务器返回的支付凭证
	...
	如果您自行调用Payright API，需要您自行解析返回的支付凭证，格式如下：
...
5. 接收Webhooks通知 
	...
	支付成功之后，Payright服务器会向主动向您的服务器发送支付结果。因此，如果您想要同步知道订单状态，需要先到Payright管理平台为您的应用配置用于接收“支付通知”消息的Webhooks地址。在Webhooks配置页面您也可以测试您配置的接收地址是否正常。
	...
	```json
	{
   		"channelOrderNo": "201604072100100417021138239",
   		"orderNo": "ch_6d7654d1572849ba687509c4",
   		"status": "SUCCESS",
   		"tradeTime": 1460002482000
	}
	```
	
6. 查看支付订单

	一切配置成功之后，您就可以尝试进行真正的支付。您每发起一笔支付，都会在Payright网站的订单管理页面有记录。
